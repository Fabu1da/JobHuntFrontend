...
export const CardActions = ({ job }: { job: Job }) => {
  ...
  const [messageToRecruiter, setMessageToRecruiter] = useState<
    Map<string, string>
  >(new Map());
  const [coverLetters, setCoverLetters] = useState<Map<string, string>>(
    new Map(),
  );
  ...
  const generateMessageForJob = async (job: Job) => {
    if (!profile) {
      setError(
        "Please upload your CV first to generate a message to the recruiter.",
      );
      return;
    }
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/api/generateMessageToRecruiter`,
        {
          profile: profile,
          job: {
            id: job.id,
      ...
      setMessageToRecruiter((prev) =>
        new Map(prev).set(job.id, response.data.message),
      );
    } catch (err) {
  ...
  const generateCoverLetterForJob = async (job: Job) => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/api/generateCoverLetter`,
        {
          profile: profile,
          job: {
            id: job.id,
      ...
      setCoverLetters((prev) =>
        new Map(prev).set(job.id, response.data.coverLetter),
      );
    } catch (err) {
    ...
    <Fragment>
      ...
      <div className="card-footer-links">
        {profile && (
          ...
          >
            {generatingCoverLetter.has(job.id)
              ? "Generating Letter..."
              : "Generate Letter"}
          </button>
        )}
      ...
      {coverLetters.has(job.id) && (
        <div className="cover-letter-section">
          <div className="cover-letter-header">
            <span className="cover-letter-title">
          ...
          <div className="cover-letter-content">{coverLetters.get(job.id)}</div>
          <button
            className="btn-copy"
            onClick={() => {
